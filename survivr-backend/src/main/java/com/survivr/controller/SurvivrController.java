...
public class SurvivrController {
    ...
    public Mono<List<LifeHack>> getLife() {
        ...
        String request = """
        {
          ...
          "messages": [{
            "role": "user",
            "content": "Return ONLY a JSON array (starting with []) containing 9 life-saving survival tips in strict JSON format. Each must include: 1. title, 2. bio (175 - 180 characters ONLY, no shorter, no longer. Add a FULL STOP at the end of the bio.), 3. image (use a realistic URL), 4. category (choose from 'Survival', 'Health', 'City Safety', 'Prep'). Do NOT include a 'content' field. Make sure that the bio itself contains the actual tip, not a tagline. No wrapper objects, just the array."
          }]
        }
        """;
